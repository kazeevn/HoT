...
def get_pose2D(video_path, output_dir):
    ...
    with torch.no_grad():
        ...
        keypoints, scores = hrnet_pose(video_path, det_dim=480, num_person=2, gen_output=True)
...
def img2video(video_path, output_dir):
    ...
    videoWriter = cv2.VideoWriter(output_dir + video_name + '.mp4', fourcc, fps, size) 
...
def get_pose3D(video_path, output_dir, fix_z):
    ...
    for i in trange(n_chunks):
        ...
        per_person_input_2D_no = []
        per_person_post_out = []
        for per_person_keypoints in keypoints:
            if pad_left != 0 or pad_right != 0:
                input_2D_no = np.pad(per_person_keypoints[low_index:high_index], ((pad_left, pad_right), (0, 0), (0, 0)), 'edge')
            else:
                input_2D_no = per_person_keypoints[low_index:high_index]
            ...
            print(output_3D_non_flip[0, 0, 0, :])
            ...
            #output_3D[:, :, 0, :] = 0
            post_out = output_3D[0].detach().cpu().numpy()

            if i == 0:
                output_3d_all = post_out
            else:
                output_3d_all = np.concatenate([output_3d_all, post_out], axis = 1)
            
            per_person_input_2D_no.append(input_2D_no)
            per_person_post_out.append(post_out)

        ## h36m_cameras_extrinsic_params in common/camera.py
        rot =  [0.1407056450843811, -0.1500701755285263, -0.755240797996521, 0.6223280429840088]
        rot = np.array(rot, dtype='float32')
        for post_out_index, post_out in enumerate(per_person_post_out):
            per_person_post_out[post_out_index] = camera_to_world(post_out, R=rot, t=0)
        ...
        for j in range(low_index, high_index):
            ...
            for input_2D_no in per_person_input_2D_no:
                image = show2Dpose(input_2D_no[jj], copy.deepcopy(image))

            output_dir_2D = output_dir +'pose2D/'
            os.makedirs(output_dir_2D, exist_ok=True)
            cv2.imwrite(f"{output_dir_2D}{j:04d}_2D.png", image)

            ## 3D
            fig_3D = plt.figure(figsize=(5.4 * len(per_person_post_out), 5.4))
            gs = gridspec.GridSpec(1, len(per_person_post_out),
                                   figure=fig_3D, wspace=0, hspace=0)

            # reversed() for demo purposes
            for idx, post_out in enumerate(reversed(per_person_post_out)):
                ax = fig_3D.add_subplot(gs[idx], projection='3d')
            ...
            fig_3D.savefig(f"{output_dir_3D}{j:04d}_3D.png", dpi=200, format='png', bbox_inches='tight')
    ...
    print('Generated 3D pose successfully!')
    ...
    print('\nGenerating demo...')
    for i in trange(len(image_2d_dir)):
...
if __name__ == "__main__":
    ...
    print('Generated demo successfully!')
